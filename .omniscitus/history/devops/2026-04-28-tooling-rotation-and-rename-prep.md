...
### 2026-04-29
**Focus**: /follow-up 검증 + 잔재 정리 PR로 unit 완결

- **/follow-up 검증**: 폴더 리네임 5단계 모두 실행 완료 확인 — cwd `/Users/mch/Documents/GitHub/budgetroad` ✓ / 메모리 디렉토리 `~/.claude/projects/-Users-mch-Documents-GitHub-budgetroad/` ✓ / `vercel project ls`에서 `budgetroad` ✓ / 현재 세션 정상 동작 ✓.
- **로컬 `.vercel/project.json` 상태**: `projectName: "wedding-budget"` 캐시 잔존하나 `projectId` 동일이라 동작 무관. 필요 시 `vercel link` 재실행으로 동기화 가능.
- **잔재 정리 PR (#23, branch `chore/rename-residue-cleanup`)**: `package.json` name `fearnot-ai` → `budgetroad`, `CLAUDE.md` 디렉토리 트리(line 40) `wedding-budget/` → `budgetroad/`. 5 files, +17 -11.
- **의도적 보존**: `.omniscitus/history/...` 과거 기록의 `wedding-budget` 텍스트는 *시점별 사실*이라 미수정 (리브랜드 의사결정의 맥락 보존).
- **의도적 미수정**: `.claude/settings.local.json`의 죽은 권한 캐시 6줄은 글로벌 gitignore + 옛 경로의 1회성 backup 작업이라 작동 무관, 별도 정리 안 함.
- **선행 절차**: PR 생성 전 origin/main에 미푸시 3개 commit 푸시(`84fe169`까지) → /follow-up tracking 업데이트 main commit(`605863f`) → branch off → cleanup commit(`78ab2ee`) → push + PR.

**Learned**:
- **잔재 정리 = 살아있는 설정에만 적용** — history records는 시점별 진실의 기록물이라 사후 리비전하면 의사결정 맥락이 소실됨. "현재 설정의 정합성"과 "기록의 정확성"은 다른 축이고, 잔재 정리는 전자에만 해당.
- **PR 만들기 전 origin/main 동기화는 강한 원칙** — 미푸시 commit이 있는 채로 branch off하면 PR diff에 무관 commit이 끼어 리뷰 노이즈가 됨. 작은 PR일수록 base가 깔끔해야 5분 PR이 진짜 5분.
- **로컬 `.vercel/project.json`은 표시용 캐시일 뿐** — `projectId`가 일치하면 모든 vercel CLI 작업이 정상. `projectName` 갱신은 cosmetic하고 `vercel link`로 언제든 동기화 가능.

## Pending
- [x] 폴더 리네임 실행 (별도 셸): `mv ~/Documents/GitHub/wedding-budget ~/Documents/GitHub/budgetroad` + `~/.claude/projects/-Users-mch-Documents-GitHub-wedding-budget` → `...-budgetroad` 동시 이전 ✔️ 2026-04-29 검증 (cwd + 메모리 디렉토리 모두 `budgetroad` 경로 확인)
- [x] Vercel 프로젝트 이름 변경 (`wedding-budget` → `budgetroad`) ✔️ 2026-04-29 검증 (`vercel project ls`에서 `budgetroad` 확인). 로컬 `.vercel/project.json`에 옛 `projectName` 캐시 남아있으나 `projectId` 동일이라 동작 무관 — 필요 시 `vercel link` 재실행으로 갱신
- [x] 새 폴더에서 Claude Code 재시작 ✔️ 2026-04-29 검증 (현재 세션이 `budgetroad` 경로에서 정상 동작)
- [x] (선택) `package.json` name 정리: `fearnot-ai` → `budgetroad` ✔️ 2026-04-29 (PR `chore/rename-residue-cleanup`)
- [x] (선택) `CLAUDE.md` 디렉토리 트리(line 40) `wedding-budget` → `budgetroad` ✔️ 2026-04-29 (PR `chore/rename-residue-cleanup`). `.omniscitus/history/...` 과거 기록은 의도적으로 보존(시점별 사실의 정확성). 로컬 `.claude/settings.local.json`은 글로벌 gitignore + 죽은 권한 캐시라 작동 무관 — 별도 손대지 않음
