...
### 2026-04-12
**Focus**: GA4 연동 + OG 이미지 생성
- `.env.local`에 `NEXT_PUBLIC_GA_ID=G-B7F0E8527V` 설정
- `src/app/opengraph-image.tsx` 생성 — Next.js ImageResponse로 동적 OG 이미지 렌더링 (Primary #FF8400, 랜딩 헤드카피 반영)
- `layout.tsx`에 `metadataBase` 추가 (배포 URL 기준 OG 이미지 절대경로 생성)
- GA4 실시간 미작동 디버깅 → 측정 ID 오타 발견 (G-B7F0E8522V → G-B7F0E8527V) → 수정 후 정상 확인

**Learned**: GA4 측정 ID는 스크린샷에서 읽지 말고 반드시 복사해서 사용할 것. 비슷한 문자(2/7)를 혼동하기 쉬움

## Pending
- [x] GitHub CI/CD 설정 (`/8-github-ci-cd-setup`)
- [x] Vercel 배포 (`/9-deploy`)
- [x] GA 측정 ID 환경변수 설정 (`.env.local`에 `NEXT_PUBLIC_GA_ID`)
- [x] OG 이미지 제작 및 추가
