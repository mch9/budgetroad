- 버짓로드 이벤트 Taxonomy v2 — 지표 → 이벤트 설계 (팀 공유) 6월1일 오후 3시 00분
    ...
    **연관 문서**
    ...
    - 신설/확장할 이벤트 **10종** + 모든 이벤트 공통으로 `session_id` 부착.
    ...
    | 재생성 시 항목별 변경 비중 | `result_reset_clicked` 직전 `care_option_toggled` 변동 집계 | 🔴 토글 + ✅ 리셋 | ✅ |
    ...
    **예시 — 한 세션의 행동 흐름**`result_viewed → tab_viewed(care) → care_option_toggled ×2 → tab_viewed(itemized) → scroll_depth(itemized,100) → result_exited(45s, last_tab=itemized)`
    ...
    **SQL 패턴** (세션별 경로 배열 → 페르소나별 집계)
    ...
    | 13 | `result_scroll_depth` | 🔴 신설 | 탭별 스크롤 도달 | `tab`, `depth_pct`(25/50/75/100), **`persona`** |
    | 14 | `result_exited` | 🔴 신설 | 결과 이탈(visibilitychange/pagehide) | `time_on_result_sec`, `last_tab`, **`persona`** |
    | 15 | `share_action_clicked` | 🔴 신설 | 저장/공유/상담 클릭 | `method`(pdf/image/link/expert), **`persona`** |
    | 16 | `result_reset_clicked` | 🟡 확장 | 다시하기 | **+ `persona`** |
    | 17 | `shared_result_viewed` | ✅ 기존 | 공유 링크 진입 | — |
    | 18 | `feedback_rated` / `feedback_submitted` | ✅ 기존 | 만족도/의견 | `rating`(+`comment`), `persona`, `region`, `season`, `total_budget` |
    
    > **method 매핑(현재 UI 기준)**: PDF로 내려받기 → `pdf` / 이미지로 저장하기 → `image` / 결과 공유하기(링크) → `link` / 전문가 상담 신청하기 → `expert`. ('저장 & 공유' 버튼은 모달을 여는 진입점 — 필요 시 `share_panel_opened`로 별도 추적 가능.)
    ...
    **항목별 내역 드롭다운**: 5개 카테고리 각각의 펼침 버튼(`tab-itemized.tsx:99`). `category`로 **어느 항목의 상세 내역을 가장 많이 열어보는지** 분석. PDF/이미지 캡처용 `forceExpand` 자동 전개는 사용자 행동이 아니므로 **제외**(실제 onClick에만 발화).
    ...
    | 저장/공유/상담 | `result-view.tsx` (`handleShareAction`) | `share_action_clicked` |
    ...
    - 절감 레버: ① `is_dev` 행 주기 삭제 ② 오래된 raw 이벤트 아카이브(집계만 보존) ③ `result_scroll_depth` 마일스톤 4→2개(50/100)로 스크롤 이벤트 ~절반 감축.