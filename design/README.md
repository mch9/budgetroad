...
### 하이브리드 경계 (2026-04-24 CTO Council)

타이포 토큰과 Figma 원안 px는 **경쟁 관계가 아니라 레이어가 다름**. 토큰은 "반응형 규칙(clamp)", px는 "한 순간의 그리드 계산". 따라서 화면 성격에 따라 둘 중 어디에 맞출지 결정한다.

| 영역 | 접근 | 이유 |
|------|------|------|
| 랜딩 페이지·입력 스텝·범용 컴포넌트 | `text-h1`~`text-caption` 토큰 | 반응형 자동·시스템 일관성 우선 |
| **결과 페이지** (`/budget-draft` 결과 뷰) | **Figma 원안 px 고정** | 디자이너가 그리드 제약 아래 계산한 값(11, 13.57, 15.5, 25.6, 42, 65 등)은 하드코딩으로 재현 |
| Dialog 내부·1회성 레이아웃 | 현재 px 유지 | 재사용 빈도 낮아 토큰화 ROI 낮음 |

**판단 기준**: "왜 이 값인가"에 근거(그리드·디자이너 의도)가 있으면 px 고정, 없으면 토큰.

**경계는 진화한다**: 초기엔 결과 페이지만 px 고정. 반복 사용 패턴이 축적되면 토큰 스케일에 흡수. 디자인 시스템은 **완성이 아니라 진화**.

기존 하드코딩된 사이즈(`src/app/page.tsx`, `src/app/budget-draft/page.tsx`)는 이 원칙에 따라 유지. **새 코드는 토큰부터 검토하되, 결과 페이지 같은 섬세 디자인 영역은 Figma px 그대로 써도 된다.**
